...
$(document).ready(function () {
    ...
    $('#radio_box').change(function () {
        ...
        if (selected_value == "5") {
            ...
            $.ajax({
                url: '../Admin/GetUserByRoleId?roleId=5',
                type: 'GET',
                success: function (d) {
                    $childUser.empty();
            ...
            $.ajax({
                url: '../Admin/GetUserByRoleId?roleId=4',
                type: 'GET',
                success: function (d) {
                    $parentUser.empty();
        ...
        if (selected_value == "4") {
            ...
            $parentUser.append($('<option></option>').val('').html('Please Wait...'));
            $.ajax({
                url: '../Admin/GetUserByRoleId?roleId=2',
                type: 'GET',
                success: function (d) {
                    $parentUser.empty();
        ...
        success: function (result) {
            ...
            if (json.message == "no data found") {
                ...
                $.each(json.data, function (key, item) {
                    ...
                    html += '<td>' + (item.role_name == null ? 'Not Define' : item.role_name) + '</td>';
...
function validateForm() {
    ...
    if (!firstName) {
        document.getElementById('checkFirstName').textContent = 'First Name is required.';
        isValid = false;
    } else {
        document.getElementById('checkFirstName').textContent = '';
    }

    if (!lastName) {
        document.getElementById('checkLastName').textContent = 'Last Name is required.';
        isValid = false;
    } else {
        document.getElementById('checkLastName').textContent = '';
    }

    if (!mobile || !/^\d{10}$/.test(mobile)) {
        document.getElementById('checkMobile').textContent = 'Valid Mobile Number is required.';
        isValid = false;
    } else {
        document.getElementById('checkMobile').textContent = '';
    }

    if (!password) {
        document.getElementById('checkPassword').textContent = 'Password is required.';
        isValid = false;
    } else {
    ...
    if (!city) {
        document.getElementById('checkCity').textContent = 'City is required.';
        isValid = false;
    } else {
        document.getElementById('checkCity').textContent = '';
    }

    if (role==0) {
        document.getElementById('checkRole').textContent = 'Role is required.';
        isValid = false;
    } else {
        document.getElementById('checkRole').textContent = '';
    }
...
function AssignUserRole(id) {
    ...
    $.ajax({
        url: "../Admin/GetUserById?user_id=" + id + "",
        method: "GET",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        success: function (result) {
...
function ShowTotalCollection() {
    ...
    $.ajax({
        url: "../Admin/GetTotalCollection?ticket_draw_date=" + $('#total_col_ticket_draw_date').val() + "",
        method: "GET",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        success: function (result) {
...
function clearTextBox() {
    ...
    $('#userRole').css('border-color', 'lightgrey');
    $("#checkFirstName").hide();
    $("#checkLastName").hide();
    $("#checkMobile").hide();
    $("#checkPassword").hide();
...
function validateUser() {
    ...
    if ($('#userRole').val().trim() == "0") {
        $('#userRole').css('border-color', 'Red');
        $("#checkRole").show();
        $("#checkRole").html("Select Role");
...
function ShowTicketSellerInfo() {
    ...
    $.ajax({
        url: "../Admin/SearchTicketNumberSellerInfo?ticket_draw_date_from=" + ticket_draw_date_from + "&ticket_draw_date_to=" + ticket_draw_date_to + "&ticket_serial_no_from=" + ticket_serial_no_from + "&ticket_serial_no_to=" + ticket_serial_no_to,
        method: "GET",
        contentType: "application/json;charset=utf-8",
        dataType: "json",
        success: function (result) {
